...
function Console(client, actor) {
  ...
  this.on("reflowActivity", this.onReflowActivity.bind(this));
}

Console.prototype = extend(ClientMethods, {
  types: ["PageError", "ConsoleAPI", "ReflowActivity"],
   ...
   *   start - timestamp when reflow started, in milliseconds since
   *           navigationStart (accurate to 1/1000 of a ms)
   *   end - timestamp when reflow ended, in milliseconds since
   *         navigationStart (accurate to 1/1000 of a ms)
   *   sourceURL - if given, precise which JS code caused this reflow
   *   sourceLine - integer
   *   functionName - string
   */
  onReflowActivity: function (event) {
    this.emit("reflow", {
      start: event.start,
